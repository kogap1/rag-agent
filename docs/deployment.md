...
MCP Streamable HTTP 端点为 `/mcp/`，提供三个只读工具：`query_knowledge_base`、`list_knowledge_documents`、`knowledge_base_health`。REST 可使用 `X-API-Key`，MCP 客户端建议使用 `Authorization: Bearer <API_KEY>`。
...
当前 LangGraph 1.x、MCP 2.x 和 LangChain 1.x 需要 Python 3.10+；官方 `torch+cu111` 停留在 PyTorch 1.10.1，不能与当前 Transformers 4.57 依赖闭环共存。因此，服务器显示 CUDA 11.1 时，脚本安装 PyTorch 2.6 官方 cu118 wheel，依靠 NVIDIA CUDA 11.x minor compatibility 运行，并在启动前实际分配 GPU 张量验证。
...
PUBLIC_HOST=rag.example.com ./run_server.sh
```

脚本会自动生成并保存 API Key，但只复用当前已激活的非 `base` Conda 环境，不会创建环境、安装包或升级依赖。也可执行 `CONDA_ENV_NAME=<已有环境名> ./run_server.sh` 显式指定环境。启动前会做离线依赖导入和 CUDA 检查；不满足时直接停止并列出原因。`PUBLIC_HOST` 会加入 MCP host allowlist；如果经 HTTPS 反向代理访问，还需在 `.env` 的 `MCP_ALLOWED_ORIGINS` 中加入实际来源，例如 `https://rag.example.com`。

```bash
tail -f runs/uvicorn.log
kill "$(cat runs/uvicorn.pid)"
```

`create_conda_env.sh` 仅保留为手工建环境工具，`run_server.sh` 不会调用它。若不希望联网，请不要执行该脚本。无 GPU 的测试机启动时可显式设置 `REQUIRE_CUDA=0`，生产启动默认禁止静默回退 CPU。

## 4. Docker（可选）

```bash
DEPLOY_MODE=docker ./run_server.sh
